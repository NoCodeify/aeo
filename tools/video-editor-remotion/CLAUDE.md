...
**Why dual-pass:** Universal-3-Pro has better word accuracy and catches false starts/repeats that U2 misses. But U3P doesn't support `disfluencies: true`, so fillers go undetected. Universal-2 catches fillers but misses false starts. The merge gives accurate words + filler detection.
...
**Prerequisites:** ffmpeg installed, ASSEMBLYAI_API_KEY env var set
...
```bash
# 1. Transcribe speaker video (run once per video)
ASSEMBLYAI_API_KEY=... npx ts-node transcribe.ts <video_dir>

# 2. Auto-cut (agent generates cuts.json, then apply)
npx ts-node autocut.ts <video_dir>

# 3. Start dev server with live preview
npm run dev

# 4. Test render (specific frames, 4K, 40Mbps)
npx remotion render MainVideo out/test.mp4 --frames=0-89 --video-bitrate=40M --props='{"config":{...}}'

# 5. Full render via script (auto-detects speaker-clean.mp4)
